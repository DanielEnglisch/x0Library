...
public class SimpleUDP implements Runnable{
	...
	private void processPacket(Packet p)
	{
		...
		{
		case Packet.STRING: receiveString(p.getString());
		break;
		case Packet.INT: receiveInt(p.getInt());
		break;
		case Packet.DOUBLE: receiveDouble(p.getDouble());
		break;
		case Packet.FLOAT: receiveFloat(p.getFloat());
		break;
		}
	}
	...
	private void receiveDouble(double d)
	{
		for(UDPReceiver r : receivers)
		{
			r.receiveDouble(d);
	...
	private void receiveFloat(float f)
	{
		for(UDPReceiver r : receivers)
		{
			r.receiveFloat(f);
